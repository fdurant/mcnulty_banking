...
d3.select('#avgCostPerContactSlider')
    .call(d3.slider()
	  //	  	  .axis(true)
	  .min(5)
	  .max(50)
	  ...
	  .on("slide", function(evt, value) {
		  d3.select('#avgCostPerContactSliderText').text(value);
		  recomputeAndRedraw();
	      }));

var defaultAvgRevenuePerContact = 50;
d3.select('#avgRevenuePerContactSlider')
    .call(d3.slider()
	  //	  	  .axis(true)
	  .min(10)
	  .max(100)
	  ...
	  .step(10)
	  .on("slide", function(evt, value) {
		  d3.select('#avgRevenuePerContactSliderText').text(value)
		  recomputeAndRedraw();
	      }));
...
d3.json("ProfitCurve.json", function(error, json) {
	...
	*/
	...
	for (var modelName in json) {
		...
		values.push({x: calculatedProfits[i][0], y: calculatedProfits[i][3], intervalProfit: calculatedProfits[i][6]});
	    }
